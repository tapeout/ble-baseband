...
class WhiteningTests(c: Whitening) extends PeekPokeTester(c) {
   ...
   var init_seed = "1111111"
   poke(c.io.init,true.B)
   poke(c.io.seed,init_seed.toInt)
   
   var init_res = goldenModel.Whitening_sw(true,0,init_seed)
   var out = init_res._1
   var s = init_res._2
   ...
   for (i <- 1 to 1) {
      ...
      out = res._1
      s = res._2
      println(s"Print during testing: Output is ${peek(c.io.result.bits)}")
